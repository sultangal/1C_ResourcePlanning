...
&НаКлиенте
Процедура КомандаПроектаЗанятостьНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Массив = Новый Массив;
	Массив.Добавить(Тип("СтандартныйПериод"));
	Элементы.КомандаПроекта.ТекущиеДанные.ИнтервалыЗанятостиСписок.ТипЗначения = Новый ОписаниеТипов(Массив);
КонецПроцедуры
...
Процедура ЗаполнитьДиаграммуГанта()
	...
	ЗанятостьНаПроектеДиаграмма.УстановитьПолныйИнтервал(НачалоМесяца(Объект.ДатаНачала), 
