...
Процедура ПроверкаПересеченияСотрудниковСГрафикомОтпусков()
	//TODO: Надо изменить график отпусков, чтобы в нем указывать только год на который график составляется
	//Дальше в запросе указывать за какие года нужен срез графика, можно отправлять в параметры даты проекта
