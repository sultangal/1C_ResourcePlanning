...
//1.Реализовать регистр сведений для производственного календаря.
//2.Добавить в таблицу команды поле средняя ставка которое будет брать данные из справочника должности
...
&НаКлиенте
Перем СписокРабочихНерабочихДней;
#КонецОбласти
#Область ОбработчикиСобытийФормы
&НаКлиенте
Процедура ПриОткрытии(Отказ)
	ОбнулимВсеПризнаки();
	ОбновитьЗначДолжнИКолвоРабДнейВТаблИнтервалов();
	СписокРабочихНерабочихДней = Новый Соответствие;
	ЗаполнСписокРабНерабДнейЗначПоУмолч(СписокРабочихНерабочихДней);
	ОбновитьСписокРабочихНерабочихДней(СписокРабочихНерабочихДней);
...
Процедура КомандаПроектаСотрудникНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	ПредыдущийСотрудник = Элементы.КомандаПроекта.ТекущиеДанные.Сотрудник;
	СтандартнаяОбработка = Ложь;
...
Процедура КомандаПроектаСотрудникПриИзменении(Элемент)
	//Проверяем нет ли уже этого сотрудника в команде
	ТекущиеДанные = Элементы.КомандаПроекта.ТекущиеДанные;
	Для Каждого Строка Из Объект.КомандаПроекта Цикл
		Если ТекущиеДанные.НомерСтроки = Строка.НомерСтроки Тогда
			Продолжить;
		КонецЕсли;
		Если Строка.Сотрудник = ТекущиеДанные.Сотрудник Тогда
			ТекущиеДанные.Сотрудник = ПредыдущийСотрудник;
			ПоказатьПредупреждение( , "В команде проекта уже есть этот сотрудник!");
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
//	Очищаем сотрудника, т.к. надо будет заново выбирать сотрудника в соответствии с введенной должностью 
...
Процедура ИнтервалыЗанятостиСотрудниковНачалоИнтервалаПриИзменении(Элемент)
	...
	КонецЕсли;
	//---------------------------------------------------------
	ОбновитьПризнакПересеченияИнтервалаСОтпуском();
КонецПроцедуры
...
&НаКлиенте
Процедура СтадииПроектаПередУдалением(Элемент, Отказ)
	Если Элементы.СтадииПроекта.ТекущиеДанные.НачалоСтадии <= НачалоДня(ТекущаяДата()) Тогда
		ПоказатьПредупреждение( , "Нельзя удалять стадию задним числом!");
		Отказ = Истина;
	КонецЕсли;
КонецПроцедуры
#КонецОбласти
...
&НаКлиенте
Процедура ОбновитьЗначДолжнИКолвоРабДнейВТаблИнтервалов()
	//Обновляем значение сотрудника в списке интервалов
	Для Каждого Сотрудник Из Объект.КомандаПроекта Цикл
		Для Каждого ИнтервалЗанятости Из Объект.ИнтервалыЗанятостиСотрудников Цикл
			Если ИнтервалЗанятости.ИД = Сотрудник.ИД Тогда
				ИнтервалЗанятости.Должность = Сотрудник.Должность;				
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Процедура ЗаполнСписокРабНерабДнейЗначПоУмолч(СписокРабочихНерабочихДней)
	//Заполним Список значениями по умолчанию (выходные - нерабочие, остальные - рабочие)
	...
	Пока День <= НачалоДня(Объект.ДатаОкончания) Цикл
		Если ДеньНедели(День) = 6 Или ДеньНедели(День) = 7 Тогда
			СписокРабочихНерабочихДней.Вставить(День, ПредопределенноеЗначение("Перечисление.СтатусДня.Нерабочий"));
		Иначе
			СписокРабочихНерабочихДней.Вставить(День, ПредопределенноеЗначение("Перечисление.СтатусДня.Рабочий"));
		КонецЕсли;
		День = День + (24 * 60 * 60);
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Процедура ОбновитьПризнакПересеченияИнтервалаСОтпуском()
	Для Каждого Сотрудник Из Объект.КомандаПроекта Цикл
		Для Каждого ИнтервалЗанятости Из Объект.ИнтервалыЗанятостиСотрудников Цикл
			Если ИнтервалЗанятости.ИД = Сотрудник.ИД Тогда
				Если (ИнтервалЗанятости.НачалоИнтервала <= ИнтервалЗанятости.ОтпускКонец И ИнтервалЗанятости.КонецИнтервала
					>= ИнтервалЗанятости.ОтпускНачало) Тогда
...
&НаКлиенте
Процедура РассчитатьЗатратыНаПроект()
	//TODO: Нужно подумать как релизовать с помощью регистра расчета
	ОбщиеЗатраты = 0;
	ЗатратыНаТекущийДень = 0;
	СписокДолжностей = ВернутьСписокДолжностейИСтавок();
	
//	//Обходим таблицу интервалов и список: должность + ставка
//	Для Каждого ИнтервалЗанятости Из Объект.ИнтервалыЗанятостиСотрудников Цикл
//		Для Каждого Должность Из СписокДолжностей Цикл
//			Если Не Должность.Ключ = Строка(ИнтервалЗанятости.Должность) Тогда
//				Продолжить;
//			КонецЕсли;
//			КоличествоДнейВИнтервале = (ИнтервалЗанятости.КонецИнтервала - ИнтервалЗанятости.НачалоИнтервала) / (24 * 60
//				* 60);
//			СтоимостьИнтервала = КоличествоДнейВИнтервале * Должность.Значение * (ИнтервалЗанятости.ПроцентЗанятости / 100);
//			День = СписокРабочихНерабочихДней.Получить();
//			ОбщиеЗатраты = ОбщиеЗатраты + СтоимостьИнтервала;
//		КонецЦикла;
//	КонецЦикла;
КонецПроцедуры

&НаСервере
Функция ВернутьСписокДолжностейИСтавок()
	Список = Новый Соответствие;
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Должности.СредняяСтавкаВСутки КАК СредняяСтавкаВСутки,
	...
	Пока Выборка.Следующий() Цикл
		Список.Вставить(Выборка.Наименование, Выборка.СредняяСтавкаВСутки);
	КонецЦикла;
...
Процедура ОпределитьТекущийСтатусПроекта()
	...
	Если ТекущаяДата >= Объект.ДатаОкончания Тогда
		...
		Возврат;
	КонецЕсли;
	Если Объект.СтадииПроекта.Количество() <= 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОбнулимВсеПризнаки()
	...
	Для Каждого Интервал Из Объект.ИнтервалыЗанятостиСотрудников Цикл
		...
		Интервал.ОтпускНачало = Дата('00010101');
		Интервал.ОтпускКонец = Дата('00010101');
	КонецЦикла;
КонецПроцедуры
			...
			Для Каждого ИнтервалЗанятости Из Объект.ИнтервалыЗанятостиСотрудников Цикл
				Если ИнтервалЗанятости.ИД = Сотрудник.ИД Тогда
					Если (ИнтервалЗанятости.НачалоИнтервала <= КонецОтпуска И ИнтервалЗанятости.КонецИнтервала
						>= НачалоОтпуска) Тогда
						ИнтервалЗанятости.ПризнакПересеченияСОтпуском = Истина;
						ИнтервалЗанятости.ОтпускНачало = НачалоОтпуска;
						ИнтервалЗанятости.ОтпускКонец = КонецОтпуска;
					Иначе
						ИнтервалЗанятости.ПризнакПересеченияСОтпуском = Ложь;
					КонецЕсли;
				КонецЕсли;
...
Процедура ОчиститьИнтервалы(ИД)
	//Добавляем в массив строки на удаление
...
Процедура ОбновитьТочкиИИнтервалы()
	...
	Для Каждого Запись Из Объект.КомандаПроекта Цикл
		...
		Если Не Запись.Сотрудник.Пустая() Тогда
			Точка.Текст = Запись.Должность.Наименование + ": " + Запись.Сотрудник.Наименование;
		...
		Для Каждого ИнтервалЗанятости Из Объект.ИнтервалыЗанятостиСотрудников Цикл
			...
			Если Запись.ИД = ИнтервалЗанятости.ИД Тогда
				...
				Если ИнтервалЗанятости.ПризнакПересеченияСОтпуском Тогда
					...
					ИнтервалОтпуска = Диаграмма.ПолучитьЗначение(Точка, Серия).Добавить();
					ИнтервалОтпуска.Начало = ИнтервалЗанятости.ОтпускНачало;
					ИнтервалОтпуска.Конец = КонецДня(ИнтервалЗанятости.ОтпускКонец);
					ИнтервалОтпуска.Текст = "Отпуск: " + Формат(ИнтервалЗанятости.ОтпускНачало, "ДЛФ=Д") + " - "
						+ Формат(ИнтервалЗанятости.ОтпускКонец, "ДЛФ=Д");
					ИнтервалОтпуска.Цвет = WebЦвета.Желтый;
				Иначе
...
Процедура ИнициализироватьДиаграммуГанта()
	...
	ВыходнойИлиПраздничный = Новый Цвет(230, 230, 230);
...
//			ВыделениеЦветом.Цвет = ВыходнойИлиПраздничный;
//		КонецЕсли;
//		День = День + (24 * 60 * 60);
//	КонецЦикла;
	//Выделить цветом выходные и праздничные дни
	Для Каждого Запись Из СписокРабочихНерабочихДней Цикл
		Если Запись.Значение = ПредопределенноеЗначение("Перечисление.СтатусДня.Нерабочий") Тогда
			ВыделениеЦветом = Диаграмма.ИнтервалыФона.Добавить(Запись.Ключ, Запись.Ключ + (24 * 60 * 60));
			ВыделениеЦветом.Цвет = ВыходнойИлиПраздничный;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ОбновитьСписокРабочихНерабочихДней(СписокРабочихНерабочихДней)
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РегистрПроизводственногоКалендаря.Год КАК Год,
	|	РегистрПроизводственногоКалендаря.ДатаИзмененияКалендаря КАК ДатаИзмененияКалендаря,
	|	РегистрПроизводственногоКалендаря.СтатусДня КАК СтатусДня
	|ИЗ
	|	РегистрСведений.РегистрПроизводственногоКалендаря КАК РегистрПроизводственногоКалендаря
	|ГДЕ
	|	РегистрПроизводственногоКалендаря.Год = &ДатаНачала
	|	ИЛИ РегистрПроизводственногоКалендаря.Год = &ДатаОкончания";

	Запрос.УстановитьПараметр("ДатаОкончания", НачалоГода(Объект.ДатаОкончания));
	...
	//Дополним список раб нераб дней значениями из запроса
	Пока Выборка.Следующий() Цикл
		Для Каждого День Из СписокРабочихНерабочихДней Цикл
			Если Выборка.ДатаИзмененияКалендаря = День.Ключ Тогда
				Если Выборка.СтатусДня = Перечисления.СтатусДня.Рабочий Тогда
					СписокРабочихНерабочихДней.Вставить(День.Ключ, Перечисления.СтатусДня.Рабочий);
				ИначеЕсли Выборка.СтатусДня = Перечисления.СтатусДня.Нерабочий Тогда
					СписокРабочихНерабочихДней.Вставить(День.Ключ, Перечисления.СтатусДня.Нерабочий);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
