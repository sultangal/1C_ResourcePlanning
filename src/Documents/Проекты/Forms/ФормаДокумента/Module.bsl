...
Процедура ПриОткрытии(Отказ)
	...
	ВернутьСписокПарДатаСтатусДня(СписокРабочихНерабочихДней);
...
Процедура РассчитатьЗатратыНаПроект()
	//TODO:  Здесь создаем запрос к регистру производственного календаря, получаем список дней, 
	//которые будут выходными и праздниками. Далее создаем цикл, который проходится по всей таблице интервалов 
	//и прибавляет дневную ставку сотрудника к затратам если это рабочий день.
КонецПроцедуры
...
Процедура ИнициализироватьДиаграммуГанта()
	...
	Пока День <= НачалоДня(Объект.ДатаОкончания) Цикл
		...
		День = День + (24 * 60 * 60);
	КонецЦикла;
	//Выделить цветом праздничные дни
	Для Каждого Пара Из СписокРабочихНерабочихДней Цикл
		ВыделениеЦветом = Диаграмма.ИнтервалыФона.Добавить(Пара.Ключ, Пара.Ключ + (24 * 60 * 60));
		Если Пара.Значение = ПредопределенноеЗначение("Перечисление.СтатусДня.Нерабочий") Тогда
			ВыделениеЦветом.Цвет = ВыходнойИлиПраздничный;
		ИначеЕсли Пара.Значение = ПредопределенноеЗначение("Перечисление.СтатусДня.Рабочий") Тогда
			ВыделениеЦветом.Цвет = WebЦвета.Белый;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ВернутьСписокПарДатаСтатусДня(СписокПарДатаСтатусДня)
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		СписокПарДатаСтатусДня.Вставить(Выборка.ДатаИзмененияКалендаря, Выборка.СтатусДня);
	КонецЦикла;
КонецПроцедуры
