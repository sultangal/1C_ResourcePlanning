...
Процедура КомандаПроектаСотрудникПриИзменении(Элемент)
	//TODO: Надо проверить есть ли уже такой сотрудник в списке, если есть не позволять добавить или вообще убрать из вариантов выбора
	...
	Для Каждого Запись Из Объект.КомандаПроекта Цикл
		...
		Для Каждого ИнтервалЗанятости Из Объект.ИнтервалыЗанятостиСотрудников Цикл
			...
			Если Запись.ИД = ИнтервалЗанятости.ИД Тогда
				Если ИнтервалЗанятости.НачалоИнтервала = Дата('00010101') ИЛИ
					 ИнтервалЗанятости.КонецИнтервала  = Дата('00010101') Тогда
					Продолжить;
				КонецЕсли;
